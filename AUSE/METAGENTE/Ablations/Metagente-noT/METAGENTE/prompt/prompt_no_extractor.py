...
Summarize the following app's description into a short term/phrase introducing the app:
<APP'S_DESCRIPTION>
$extracted_text
</APP'S_DESCRIPTION>
...
TEACHER_PROMPT = """
You are a professional Prompt Engineer. You are working on a system using a Large Language Model (LLM) to help developers automatically generate a short Description term/phrase contain key concept/idea from the description of an app. Your task is to modify and improve the current prompt of the LLM based on the result of testing on a data include a description and a ground truth description.
 
# Steps:
- **Analyze the data for testing**: Analyze the following data include a description and a ground truth description from an app:
<DESCRIPTION>
$extracted_text
</DESCRIPTION>
...
- **Review the current result**: Review the generated description using the description its ROUGE score on the ground truth description to identify improvements that could be made:
...
- **Prioritize extracting existing name/title, feature description/purpose statement/overview**: Compare the text from the beginning of the description and the ground truth description. If the ground truth description is already existed in this description as a name/title, feature description/purpose statement/overview, you must include in the new prompt the instruction to prioritize using it.
...
OPTIMIZED_SUMMARIZER_PROMPT = """"Summarize the following app's description into a short term/phrase introducing the app. Prioritize using any existing name/title, feature description, or purpose statement from the beginning of the description if it encapsulates the app's key concept or idea. Ensure the output closely aligns with these elements to maintain consistency and accuracy. If a concise description of the app's core purpose or overview is present, use it as the basis for the summary:
